      ...
      <div style={{ display: 'flex', background: 'white', borderBottom: '1px solid #eee' }}>
        {['beneficiarios', 'registro', 'comercios', 'nuevo_local'].map((t, i) => (
          <button key={t} onClick={() => setTab(t)} style={{ flex: 1, padding: '16px', fontSize: 11, fontWeight: 800, border: 'none', background: 'none', color: tab === t ? '#00B5AD' : '#bbb', borderBottom: tab === t ? '4px solid #00B5AD' : '4px solid transparent', transition: '0.3s' }}>
            {['BENEFICIARIOS', 'NUEVO REGISTRO', 'COMERCIOS', 'NUEVO LOCAL'][i]}
          </button>
        ))}
      </div>
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '1.5rem' }}>
        ...
        {tab === 'comercios' && (
          <>
            ...
            <div style={{ position: 'relative', marginBottom: 20 }}>
              <input value={searchComercio} onChange={e => setSearchComercio(e.target.value)} placeholder="Buscar por nombre o categoría..." style={{ paddingLeft: 40, borderRadius: 12, height: 45 }} />
              <span style={{ position: 'absolute', left: 15, top: '52%', transform: 'translateY(-50%)', fontSize: 18 }}>🔍</span>
            </div>
            {selectedComercios.length > 0 && (
              <div style={{ background: '#222', color: 'white', padding: '14px 20px', borderRadius: 14, marginBottom: 20, display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <button onClick={() => setSelectedComercios([])} style={{ background: 'none', border: 'none', color: 'white', fontSize: 24, marginLeft: 5 }}>×</button>
              </div>
            )}
            <div style={{ display: 'flex', alignItems: 'center', padding: '0 10px 12px', gap: 12 }}>
              <input type="checkbox" checked={selectedComercios.length === filteredComercios.length && filteredComercios.length > 0} onChange={toggleSelectAllComercios} style={{ width: 20, height: 20 }} />
              <span style={{ fontSize: 11, color: '#999', fontWeight: 800 }}>SELECCIONAR TODOS</span>
            </div>
            {loadingComercios ? <div style={{ textAlign: 'center', padding: '4rem', color: '#aaa', fontWeight: 600 }}>Cargando comercios...</div> :
              filteredComercios.map((c, idx) => (
                <div key={c.id} style={{ background: 'white', border: '1px solid #eee', borderRadius: 14, padding: '15px', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 15 }}>
                  <input type="checkbox" checked={selectedComercios.includes(c.id)} onChange={() => toggleSelectComercio(c.id)} style={{ width: 20, height: 20, flexShrink: 0 }} />
                  <div style={{ width: 44, height: 44, borderRadius: '50%', background: AVATAR_COLORS[idx % AVATAR_COLORS.length], color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 800, flexShrink: 0 }}>{initials(c.nombre)}</div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 15, fontWeight: 700, color: '#333' }}>{c.nombre}</div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', zIndex: 100, backdropFilter: 'blur(5px)' }}>
          <div style={{ background: 'white', borderRadius: 20, padding: '2.5rem', width: '100%', maxWidth: 500, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 15, marginBottom: 15 }}>
              ...
              <div><label style={{ fontSize: 10, fontWeight: 800, color: '#999' }}>ESTADO</label>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', zIndex: 100, backdropFilter: 'blur(5px)' }}>
          <div style={{ background: 'white', borderRadius: 20, padding: '2.5rem', width: '100%', maxWidth: 500 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={eliminarComercio} style={{ flex: 1, background: '#fff', color: '#A0005A', border: '1px solid #A0005A', fontWeight: 700, borderRadius: 10 }}>ELIMINAR</button>
                <button onClick={() => setEditComercio(null)} style={{ flex: 1, background: '#f0f0f0', border: 'none', fontWeight: 700, borderRadius: 10 }}>CERRAR</button>
              </div>
            </div>
          </div>
        </div>
