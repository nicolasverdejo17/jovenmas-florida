...
function Footer() {
  return (
    <div style={{ background: 'linear-gradient(135deg, #00B5AD, #6B8500)', padding: '1rem 1rem 0.8rem', marginTop: '1rem' }}>
...
function DescuentoEditor({ value, onChange }) {
  const parsed = parseDescuento(value)
  const isMulti = Array.isArray(parsed)
  const [modo, setModo] = useState(isMulti ? 'multi' : 'simple')
  const [simpleVal, setSimpleVal] = useState(isMulti ? '' : (parsed ?? ''))
  const [items, setItems] = useState(isMulti ? parsed : [{ cat: '', pct: '' }])

  function emitSimple(val) {
    setSimpleVal(val)
    onChange(val)
  }

  function emitMulti(newItems) {
    setItems(newItems)
    const validos = newItems.filter(i => i.cat && i.pct !== '')
    if (validos.length > 0) {
      onChange(JSON.stringify(validos.map(i => ({ cat: i.cat, pct: parseFloat(i.pct) }))))
    }
  }
  ...
  function switchModo(m) {
    ...
    if (m === 'simple') {
      ...
      const newItems = [{ cat: '', pct: '' }]
      setItems(newItems)
      onChange('')
    }
  }

  function addItem() { emitMulti([...items, { cat: '', pct: '' }]) }
  function removeItem(i) { emitMulti(items.filter((_, idx) => idx !== i)) }
  function updateItem(i, field, val) {
    const copy = items.map((it, idx) => idx === i ? { ...it, [field]: val } : it)
    emitMulti(copy)
  }
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
        {['simple', 'multi'].map(m => (
          <button key={m} type="button" onClick={() => switchModo(m)}
            style={{ flex: 1, padding: '8px', fontSize: 11, fontWeight: 700, border: '2px solid', borderRadius: 10, cursor: 'pointer', fontFamily: 'inherit', background: modo === m ? '#00B5AD' : 'white', color: modo === m ? 'white' : '#00B5AD', borderColor: '#00B5AD' }}>
            {m === 'simple' ? '% Único' : '🗂 Múltiples'}
          </button>
        ))}
      </div>
      {modo === 'simple' && (
        <input type="number" value={simpleVal} onChange={e => emitSimple(e.target.value)} placeholder="Ej: 15"
          style={{ width: '100%', padding: 12, border: '1px solid #eee', borderRadius: 10, fontSize: 14, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' }} />
      )}
      {modo === 'multi' && (
        <div>
          {items.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems: 'center' }}>
              <input value={item.cat} onChange={e => updateItem(i, 'cat', e.target.value)} placeholder="Ej: Ropa, Calzado..."
                style={{ flex: 2, padding: '10px 12px', border: '1px solid #eee', borderRadius: 10, fontSize: 13, fontFamily: 'inherit', outline: 'none' }} />
              <input type="number" value={item.pct} onChange={e => updateItem(i, 'pct', e.target.value)} placeholder="%"
                style={{ flex: 1, padding: '10px 12px', border: '1px solid #eee', borderRadius: 10, fontSize: 13, fontFamily: 'inherit', outline: 'none' }} />
              {items.length > 1 && (
                <button type="button" onClick={() => removeItem(i)}
                  style={{ background: '#FCE8F3', border: 'none', borderRadius: 8, color: '#A0005A', fontWeight: 800, fontSize: 16, width: 34, height: 34, cursor: 'pointer', flexShrink: 0 }}>×</button>
              )}
            </div>
          ))}
          <button type="button" onClick={addItem}
            style={{ width: '100%', padding: '9px', background: '#E0F7F6', color: '#007A75', border: 'none', borderRadius: 10, fontWeight: 700, fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>
            + Agregar categoría
          </button>
        </div>
      )}
          ...
          <>
            ...
            {loadingComercios ? <div style={{ textAlign: 'center', padding: '4rem', color: '#aaa', fontWeight: 600 }}>Cargando comercios...</div> :
              filteredComercios.map((c, idx) => {
                const desc = parseDescuento(c.descuento)
                const isMulti = Array.isArray(desc)
                return (
                  <div key={c.id} className="comercio-row" style={{ background: 'white', border: '1px solid #eee', borderRadius: 14, padding: '14px', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div className="desc-badge" style={{ background: '#FCE8F3', borderRadius: 10, padding: '6px 10px', textAlign: 'center', flexShrink: 0, minWidth: 54 }}>
                      {isMulti ? (
                        <><div style={{ fontSize: 13, fontWeight: 900, color: '#D63D8F', lineHeight: 1 }}>{desc.length}</div><div style={{ fontSize: 8, fontWeight: 800, color: '#D63D8F' }}>CATEG.</div></>
                      ) : (
                        <><div style={{ fontSize: 18, fontWeight: 900, color: '#D63D8F', lineHeight: 1 }}>{desc}%</div><div style={{ fontSize: 8, fontWeight: 800, color: '#D63D8F' }}>DESC.</div></>
                      )}
                    </div>
        ...
        {tab === 'nuevo_local' && (
          <div style={{ background: 'white', border: '1px solid #eee', borderRadius: 20, padding: '1.5rem' }}>
            ...
            <div className="form-grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 15, marginBottom: 15 }}>
              <div><label style={{ fontSize: 11, fontWeight: 800, color: '#666', display: 'block', marginBottom: 5 }}>CATEGORÍA</label><input value={formComercio.categoria} onChange={e => setFormComercio({ ...formComercio, categoria: e.target.value })} placeholder="Ej: Gym, Panadería..." /></div>
              <div>
                <label style={{ fontSize: 11, fontWeight: 800, color: '#666', display: 'block', marginBottom: 5 }}>DESCUENTO(S)</label>
                <DescuentoEditor value={formComercio.descuento} onChange={val => setFormComercio({ ...formComercio, descuento: val })} />
              </div>
            </div>
            <div className="form-grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 15 }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', zIndex: 100, backdropFilter: 'blur(5px)' }}>
          <div style={{ background: 'white', borderRadius: 20, padding: '1.5rem', width: '100%', maxWidth: 500, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div className="form-grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 15, marginBottom: 15 }}>
              ...
              <div><label style={{ fontSize: 10, fontWeight: 800, color: '#999', display: 'block', marginBottom: 5 }}>ESTADO</label>
                <select value={editComercio.estado} onChange={e => setEditComercio({ ...editComercio, estado: e.target.value })}>
            ...
            <div style={{ marginBottom: 15 }}>
              <label style={{ fontSize: 10, fontWeight: 800, color: '#999', display: 'block', marginBottom: 5 }}>DESCUENTO(S)</label>
              <DescuentoEditor value={editComercio.descuento} onChange={val => setEditComercio({ ...editComercio, descuento: val })} />
            </div>
            <div className="form-grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 15, marginBottom: 15 }}>
