  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        @keyframes float { 0%, 100% { transform: translateY(0px) rotate(-5deg); } 50% { transform: translateY(-10px) rotate(-5deg); } }
      ...
      <div className="splash-card" style={{ background: 'rgba(255,255,255,0.97)', borderRadius: 32, padding: '2.5rem 2rem 2rem', width: '100%', maxWidth: 360, textAlign: 'center', boxShadow: '0 32px 64px rgba(0,0,0,0.22)', position: 'relative', zIndex: 1 }}>
        <div className="splash-logo" style={{ width: 80, height: 80, background: 'linear-gradient(135deg, #00B5AD, #007A75)', borderRadius: 24, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 22px', boxShadow: '0 12px 28px rgba(0,181,173,0.4)' }}>
        ...
        <div style={{ fontSize: 12, fontWeight: 700, color: '#00B5AD', letterSpacing: 3, textTransform: 'uppercase', marginTop: 6 }}>Florida</div>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 28 }}>
          <span style={{ background: '#E0F7F6', color: '#007A75', fontSize: 11, fontWeight: 700, padding: '5px 13px', borderRadius: 20 }}>🏪 Comercios locales</span>
          <span style={{ background: '#FCE8F3', color: '#A0005A', fontSize: 11, fontWeight: 700, padding: '5px 13px', borderRadius: 20 }}>🎉 Descuentos reales</span>
        </div>
        <button className="splash-btn" onClick={onEnter} style={{ width: '100%', padding: '17px', background: 'linear-gradient(135deg, #00B5AD, #007A75)', color: 'white', border: 'none', borderRadius: 16, fontSize: 15, fontWeight: 800, cursor: 'pointer', letterSpacing: 0.3, boxShadow: '0 8px 20px rgba(0,181,173,0.38)', fontFamily: 'inherit' }}>
          Ver comercios adheridos →
        </button>
        <div style={{ marginTop: 16, fontSize: 11, color: '#bbb' }}>@joven_mas_florida · joven.mas.florida@gmail.com</div>
      </div>
    </div>
  )
}
...
// ─── Card pública de comercio ─────────────────────────────────────────────────
function ComercioCard({ c, idx }) {
  const desc = parseDescuento(c.descuento)
  const isMulti = Array.isArray(desc)

  return (
    <div style={{ background: 'white', borderRadius: 16, padding: '18px', marginBottom: 14, boxShadow: '0 2px 12px rgba(0,0,0,0.08)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        {/* Badge: si es simple muestra %, si es multi muestra "N desc." */}
        {!isMulti && (
          <div style={{ background: 'linear-gradient(135deg, #D63D8F, #a0005a)', borderRadius: 14, padding: '10px 14px', textAlign: 'center', flexShrink: 0, boxShadow: '0 4px 12px rgba(214,61,143,0.3)' }}>
            <div style={{ fontSize: 30, fontWeight: 900, color: 'white', lineHeight: 1 }}>{desc}%</div>
            <div style={{ fontSize: 9, fontWeight: 800, color: 'rgba(255,255,255,0.85)', letterSpacing: 1 }}>DESCUENTO</div>
          </div>
        )}
        {isMulti && (
          <div style={{ background: 'linear-gradient(135deg, #D63D8F, #a0005a)', borderRadius: 14, padding: '10px 14px', textAlign: 'center', flexShrink: 0, boxShadow: '0 4px 12px rgba(214,61,143,0.3)' }}>
            <div style={{ fontSize: 22, fontWeight: 900, color: 'white', lineHeight: 1 }}>{desc.length}</div>
            <div style={{ fontSize: 9, fontWeight: 800, color: 'rgba(255,255,255,0.85)', letterSpacing: 1 }}>DESCUENTOS</div>
          </div>
        )}
      </div>

      {/* Sub-items de descuentos múltiples */}
      {isMulti && (
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#00B5AD', display: 'flex', flexDirection: 'column', fontFamily: '-apple-system, BlinkMacSystemFont, sans-serif' }}>
      ...
      <div style={{ flex: 1, padding: '0 1rem 0' }}>
        ...
        {filtered.map((c, idx) => (
          <ComercioCard key={idx} c={c} idx={idx} />
        ))}
      </div>
